...
from model_grey_Cell import LeNet
...
# Select the First two images and labels
images = TrainSet
labels = TrainLabels
indices = []
...
for i in range(5):
    ...
    # 假设图像数据的形状是(batch_size, channels, height, width) 并且是灰度的
    if len(images[index].shape) == 3 and images[index].shape[0] == 1:
        img = images[index, 0]
    else:
        img = images[index]
